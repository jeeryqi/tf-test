...
'''卷积层＃1
在我们的第一个卷积层中，我们希望将32个5x5滤波器应用于输入层，并具有ReLU激活功能。我们可以使用模块中的conv2d()方法 layers创建此层，如下所示：
该inputs参数指定了输入张量，其中必须有形状 。在这里，我们将第一个卷积层连接到具有形状的卷积层。[batch_size, image_height, image_width, channels]input_layer[batch_size, 28, 28, 1]
...
该padding参数指定了两个枚举值（不区分大小写）中的一个：valid（缺省值）或same。要指定输出张量应具有与输入张量相同的高度和宽度值，我们padding=same在此处设置，它指示TensorFlow将0值添加到输入张量的边缘以保持28的高度和宽度。（没有填充，a在28x28张量上进行5x5卷积将产生24x24张量，因为有24x24个位置从28x28网格中提取5x5瓦片。）
...
'''汇集层＃1
接下来，我们将第一个池化层连接到刚刚创建的卷积层。我们可以使用该max_pooling2d()方法layers构造一个层，该层使用2x2过滤器和2的步幅执行最大池：
...
该pool_size参数指定了最大池过滤器的大小 （这里）。如果两个维度具有相同的值，则可以改为指定单个整数（例如， ）。[height, width][2, 2]pool_size=2

该strides参数指定步幅的大小。在这里，我们设置了一个步幅为2，表示过滤器提取的子区域在高度和宽度维度上应分开2个像素（对于2x2过滤器，这意味着所提取的区域都不会重叠）。如果要为高度和宽度设置不同的步幅值，可以改为指定元组或列表（例如，stride=[3, 6]）。

我们的max_pooling2d()（pool1）产生的输出张量具有以下形状 ：2x2滤波器将高度和宽度各减少50％。[batch_size, 14, 14, 32]'''
  pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)

'''卷积层＃2和池层＃2
我们可以使用conv2d()和max_pooling2d()以前一样将第二个卷积和池化层连接到我们的CNN 。对于卷积层＃2，我们使用ReLU激活配置64个5x5过滤器，对于池＃2，我们使用相同的规范作为池＃1（2x2最大池过滤器，步幅为2）：
注意，卷积层＃2将我们的第一个池化层（pool1）的输出张量作为输入，并将张量conv2作为输出。conv2 具有与（由于）相同的高度和宽度的形状，以及用于64个滤波器的64个通道。[batch_size, 14, 14, 64]pool1padding="same"
...
池＃2 conv2作为输入，产生pool2输出。pool2 有形状（高度和宽度减少50％）。[batch_size, 7, 7, 64]conv2'''
  ...
  # Output Tensor Shape: [batch_size, 7 * 7 * 64]
'''密集层
接下来，我们想要向我们的CNN添加一个密集层（具有1,024个神经元和ReLU激活），以对卷积/池化层提取的特征进行分类。然而，在我们连接图层之前，我们会将要素贴图（pool2）展平为形状，以便我们的张量只有两个维度：[batch_size, features]
在reshape()上面的操作中，-1表示batch_size 将根据输入数据中的示例数量动态计算维度。每个示例都有7（pool2高度）* 7（pool2宽度）* 64（pool2通道）功能，因此我们希望features维度的值为7 * 7 * 64（总共3136）。输出张量pool2_flat具有形状 。[batch_size, 3136]'''
...
该inputs参数指定输入张：我们的扁平化特征图， pool2_flat。该units参数指定在致密层（1024）神经元的数目。该activation参数采用激活函数; 再次，我们将使用tf.nn.relu添加ReLU激活。'''
...
该training参数采用布尔值来指定模型当前是否在训练模式下运行; 如果training是，则仅执行丢失 True。在这里，我们检查mode传递给我们的模型函数 cnn_model_fn是否是TRAIN模式。
...
该input参数指定从中提取最大值，这里的张量logits。该axis参数指定的轴input 张量沿找到最大的价值。在这里，我们希望在索引为1的维度上找到最大值，这对应于我们的预测（回想一下我们的logits张量有形状）。[batch_size, 10]'''
      "classes": tf.argmax(input=logits, axis=1),
      # Add `softmax_tensor` to the graph. It is used for PREDICT and by the
      # `logging_hook`.
      '''我们可以通过使用softmax激活来从logits层导出概率tf.nn.softmax
      注意：我们使用name参数来明确命名此操作 softmax_tensor，因此我们稍后可以引用它。（我们将在“设置记录挂钩”中设置 softmax值的记录）。'''
...
'''配置培训操作
在上一节中，我们将CNN的损失定义为logits图层和我们的标签的softmax交叉熵。让我们配置我们的模型以在训练期间优化此损失值。我们将使用0.001的学习率和 随机梯度下降 作为优化算法：'''
...
'''设置记录钩
由于CNN可能需要一段时间才能进行训练，我们可以设置一些记录，以便我们可以在训练期间跟踪进度。我们可以使用TensorFlow tf.train.SessionRunHook创建一个 tf.train.LoggingTensorHook 记录来自CNN的softmax层的概率值。
...
接下来，我们创建LoggingTensorHook，传递tensors_to_log给 tensors参数。我们设置every_n_iter=50，它指定在每50个训练步骤之后应该记录概率。'''
...
在numpy_input_fn通话中，我们将训练特征数据和标签分别传递给 x（作为dict）和y。我们设置batch_size的100（这意味着该模型将上的100个例子minibatches培养在每一个步骤）。 num_epochs=None表示模型将训练直到达到指定的步数。我们还设置shuffle=True了改组培训数据。在train通话中，我们设置steps=20000 （这意味着模型将训练总共20,000步）。我们将我们传递 logging_hook给hooks论证，以便在训练期间触发。'''
...
'''评估模型
培训完成后，我们要评估我们的模型，以确定其在MNIST测试集上的准确性。我们调用该evaluate方法，该方法评估我们在eval_metric_ops参数中指定的指标model_fn。
